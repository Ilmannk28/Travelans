...
export default class Map {
    ...
    static async build(selector, options = {}) {
        ...
        const jakartaCoordinates = [-6.1751, 106.8650];

        if ('locate' in options && options.locate) {
            try {
                const positons = await Map.getCurrentPosition();
                const coordinate = [positons.coords.latitude, positons.coords.longitude];
                ...
                console.error('build map error:', error);

                return new Map(selector, {
                    ...options,
                    center: jakartaCoordinates,
                })
            }
        }
    }
    ...
    addMarker(coordinates, markerOptions = {}, popupOptions = nulll) {
